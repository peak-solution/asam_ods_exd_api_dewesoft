...
class TestDockerContainer(unittest.TestCase):
    ...
    def setUpClass(cls):
        # Docker-Image bauen
        subprocess.run(
            ["docker", "build", "-t", "asam-ods-exd-api-dewesoft", "."], check=True)
        ...
        cp = subprocess.run(
            ["docker", "run", "-d", "--rm", "--name", "test_container",
             "-p", "50051:50051", "-v", f"{data_folder}:/data", "asam-ods-exd-api-dewesoft"],
    ...
    def test_structure(self):
        with grpc.insecure_channel("localhost:50051") as channel:
            ...
            try:
                ...
                self.assertEqual(
                    structure.groups[0].channels[0].unit_string, " ")
                self.assertEqual(
